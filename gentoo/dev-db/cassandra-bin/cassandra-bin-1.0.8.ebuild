...
DEPEND=">=virtual/jdk-1.6 dev-java/sun-jdk"
RDEPEND=">=virtual/jre-1.6 !dev-db/cassandra-bin"
...
src_install() {
	...
	insinto /usr/share/cassandra-bin
	newins "${FILESDIR}/cassandra.in.sh" cassandra.in.sh || die "newins failed"

	# JAR libs
	java-pkg_dojar lib/*.jar || die "dojar failed"
	#java-pkg_newjar build/apache-${P}-SNAPSHOT.jar cassandra.jar || die "newjar failed"

	# Configs
	insinto /etc/cassandra
	doins conf/*.{properties,yaml,txt} || die "doins failed"
	newconfd "${FILESDIR}/conf_d_cassandra_08" cassandra || die "newconfd failed"
	cat conf/cassandra-env.sh | grep -v break | awk 'NR==121{$0=" CASSANDRA_HOME=/usr/share/cassandra-bin\n"$0}1' >> ${D}/etc/conf.d/cassandra || die "cat/grep/>> failed"

	newinitd "${FILESDIR}/init_d_cassandra_08" cassandra || die "newinitd failed"
